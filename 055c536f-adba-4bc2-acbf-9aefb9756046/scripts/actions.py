...
def defaultCardAction(args):
    ...
    if args.card.group == table:
        # Double-click resets a stat token to 0. Stepping the value stays on
        # the Up/Down arrows; mouse buttons themselves cannot be rebound in
        # OCTGN - left is select/drag, right opens the context menu - so
        # double-click is the only pointer gesture available, and clearing a
        # modifier is the gesture worth having on it.
        # Origine : Merlin - jetons de stat (2026).
        if args.card.Type == 'stat_token':
            resetStatToken(args.card)
            return
...
def resetStatToken(card, x = 0, y = 0):
    """Back to the base face, i.e. value 0."""
    mute()
    if card.alternate == "":
        return
    card.alternate = ""
